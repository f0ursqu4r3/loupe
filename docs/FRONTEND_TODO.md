...
- [x] Reload editor views on route param change (`/dashboards/:id`, `/queries/:id`, `/schedules/:id`, `/visualizations/:id`) to prevent stale state when navigating between items.
- [x] Add cancellation/guarding for long-poll loops in query and dashboard views to avoid updates after unmount and reduce stray API calls.
