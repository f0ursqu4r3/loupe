...
2. **Rate Limiting** (Not Started)
   - ⏳ Add actix-governor or custom middleware
...
3. **Dangerous function (❌ Should fail):**
```bash
curl -X POST http://localhost:8080/runs/execute \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
...
2. **Check server logs for full error with error_id:**
```bash
...
| A07:2021 Auth Failures | 🔴 FAIL | 🟡 PARTIAL | JWT fixed, rate limiting pending |
